...
import {
  ...
  CreditCard, Download, CheckCircle2, Info, ChevronRight,
} from 'lucide-react';

// ─── Stat Card ───────────────────────────────────────────────
function StatCard({ icon: Icon, label, value, trend, color }) {
  const colors = {
    indigo: { bg: 'bg-indigo-50', iconBg: 'bg-indigo-100', iconColor: 'text-indigo-600', trendColor: 'text-indigo-600' },
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex items-start gap-3 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3">
        ...
        <p className="text-sm text-indigo-700 font-medium">
          Uploaded receipts are processed by AWS Textract — amounts and merchants are extracted automatically and saved to your expense list.
        </p>
      </div>
...
function AnalyticsView() {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      {/* Summary Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard icon={Receipt}    label="Total Spent"     value={`$${total.toFixed(2)}`} color="indigo" />
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h3 className="text-base font-bold text-gray-900 mb-5">Spending by Category</h3>
      ...
      {/* Monthly Bar Chart */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h3 className="text-base font-bold text-gray-900 mb-5">Monthly Spending — 2026</h3>
              ...
              <div key={m} className="flex-1 flex flex-col items-center gap-1.5 group">
                ...
                <div
                  ...
                  className={`w-full rounded-t-lg transition-all duration-300 ${
                    isThisMonth ? 'bg-indigo-500' : 'bg-gray-200 group-hover:bg-indigo-300'
                  }`}
                  style={{ height: `${pct}%` }}
                />
...
function ToggleRow({ label, description, defaultValue = false }) {
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-1">
        ...
        <div className="py-4 border-b border-gray-50">
          <label className="block text-sm font-semibold text-gray-800 mb-1.5">Default Currency</label>
          <p className="text-xs text-gray-400 mb-3">Used for all expense amounts</p>
          <select
            value={currency}
            onChange={e => setCurrency(e.target.value)}
            className="px-3.5 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm font-medium text-gray-900
        ...
        {/* Theme */}
        <div className="py-4 border-b border-gray-50">
          <label className="block text-sm font-semibold text-gray-800 mb-1.5">Theme</label>
        ...
        <ToggleRow label="Compact View" description="Show more rows with reduced padding" />
        <ToggleRow label="Auto-refresh" description="Refresh expense list every 5 minutes" defaultValue={true} />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <ToggleRow label="Upload Alerts"       description="Notify when a receipt is processed"     defaultValue={true} />
        <ToggleRow label="Monthly Summary"     description="Email summary of spending each month"   defaultValue={true} />
        <ToggleRow label="Spending Threshold"  description="Alert when you exceed your budget goal" />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Data &amp; Privacy</h3>
        </div>

        <div className="space-y-3">
          {[
            { icon: Download,    label: 'Export all expenses as CSV', color: 'indigo' },
            { icon: CreditCard,  label: 'Manage billing & subscription', color: 'purple' },
          ].map(({ icon: Icon, label, color }) => (
            <button
              key={label}
              className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border border-gray-100
                hover:border-${color}-200 hover:bg-${color}-50 transition-all duration-150 group`}
            >
              <div className="flex items-center gap-3">
          ...
          <button className="w-full flex items-center justify-between px-4 py-3 rounded-xl border border-red-100 hover:bg-red-50 transition-all duration-150 group">
...
export default function Dashboard() {
  ...
  return (
    <div className="flex flex-1 w-full overflow-hidden">
      ...
      {/* ── Mobile Tab Bar ───────────────────────────────── */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-20 bg-white/90 backdrop-blur-xl border-t border-gray-200 flex">
        {VIEWS.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`flex-1 flex flex-col items-center gap-1 py-3 text-[10px] font-bold uppercase tracking-wide transition-colors
              ${activeId === id ? 'text-indigo-600' : 'text-gray-400 hover:text-gray-700'}`}
          >
            <Icon size={20} />
            {label}
          </button>
